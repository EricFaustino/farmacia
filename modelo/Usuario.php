<?php
    ...
    class Usuario{
        ...
        function subir($pass,$id_aumentou,$id_usuario){
            ...
            if(!empty($this->objetos)){
                ...
                $query= $this->acceso->prepare($sql);
                $query->execute(array(':id'=>$id_aumentou,':tipo'=>$tipo));
                echo 'subiu';
            }else{
                echo 'não subiu';
            }
        }
        function descer($pass,$id_diminuiu,$id_usuario){
            $sql="SELECT id_usuario FROM usuario where id_usuario=:id_usuario and contrasena_us=:pass";
            $query= $this->acceso->prepare($sql);
            $query->execute(array(':id_usuario'=>$id_usuario,':pass'=>$pass));
            $this->objetos=$query->fetchall();
            if(!empty($this->objetos)){
                ...
                $sql="UPDATE usuario SET us_tipo=:tipo where id_usuario=:id";
                $query= $this->acceso->prepare($sql);
                $query->execute(array(':id'=>$id_diminuiu,':tipo'=>$tipo));
                echo 'desceu';
            }else{
                echo 'Não desceu';
            }
        }
        function excluir($pass,$id_excluir,$id_usuario){
            $sql="SELECT id_usuario FROM usuario where id_usuario=:id_usuario and contrasena_us=:pass";
            $query= $this->acceso->prepare($sql);
            $query->execute(array(':id_usuario'=>$id_usuario,':pass'=>$pass));
            $this->objetos=$query->fetchall();
            if(!empty($this->objetos)){
                ...
                $query= $this->acceso->prepare($sql);
                $query->execute(array(':id'=>$id_excluir));
                echo 'excluir';
            }else{
                echo 'Não excluido';
            }
